...
const Timeoffer=({date,onChange,sethours,settimechoice,setminutes,setdatevalid,setindexchoice})=>{
    const hours=(number,index,item)=>{
        const list_hours=[]
        for(let i=0;i<number;i++){
            list_hours.push(<li name={i} className={`${item.hours==i?'selected':''}`} onClick={(e)=>settimeschoice(e,i,index,'hours')}>{('0'+i).slice(-2)}</li>)
        }
        return list_hours
    }
    ...
    const minutes=(number,index,item)=>{
        const list_minutes=[]
        for(let i=0;i<number;i++){
            list_minutes.push(<li key={i} className={`${item.minutes==i?'selected':''}`} onClick={(e)=>settimeschoice(e,i,index,'minutes')}>{('0'+i).slice(-2)}</li>)
        }
        return list_minutes
    }

    const scrooltimes=(e,index,name,number)=>{
       for(let i=0;i<number;i++){
            if (e.target.scrollTop>(i*32-18)&&e.target.scrollTop<(i*32+18) && e.target.scrollTop>1){
                settimechoice(i,index,name) 
            }
       }
   }
   
   useEffect(() => {
       let list_scroll_hours=document.querySelectorAll('.time-picker__hour-spinner .scrollbar__wrapper')
       let list_scroll_minutes=document.querySelectorAll('.time-picker__minute-spinner .scrollbar__wrapper')
       for(let i=0;i<list_scroll_hours.length;i++){
        list_scroll_hours[i].scrollTop=date[i].hours*32
        list_scroll_minutes[i].scrollTop=date[i].minutes*32
       }
       
  }, [date]);
    
   const settimeschoice=(e,value,index,name)=>{
        e.stopPropagation()
        settimechoice(value,index,name) 
    }
    const comfirmdate=(index)=>{
    ...
    const setindex=(e,index)=>{
        ...
        window.onclick=(event)=>{
            let parent=event.target.closest('.date_pickers.popper_content')
            ...
            <div className="form-item_control">
                {date.map((item,index)=>{
                    ...
                    <div className="picker-item start-picker">
                        <div className="rele">
                            <div onClick={(e)=>setindex(e,index)} className="hearder_choice hearder item-center" name={index==0?'valid_from':'valid_to'}>
                                ...
                                <div className="selector__inner">{item.time.toLocaleString('sv-SE', { timeZone: 'Asia/Ho_Chi_Minh' }).substr(0,10)} {item.hours<10?`0${item.hours}`:item.hours}:{item.minutes<10?`0${item.minutes}`:item.minutes}</div>
                            </div>
                            
                            <div className="date_pickers popper_content" style={{display:`${item.show?'':'none'}`}}>
                                <div className="d-flex date_picker-date">
                                    <div className="calendar-picker">
                                        ...
                                        <Calendar  
                                            ...
                                            onClickYear={(value, event) => showon(value)}
                                        ...
                                        <div className="time-picker-panel">
                                            <div className="time-picker-panel__body">
                                                ...
                                                <div className="time-spinner time-picker__hour-spinner">
                                                    <i onClick={(e)=>settimeschoice(e,item.hours-1,index,'hours')}  className={`time-spinner__icon ${item.hours==0?'disable':''} up icon`}>
                                                    ...
                                                    <div className="scrollbar">
                                                        <div onScroll={(e)=>scrooltimes(e,index,'hours',24)} ref={scrollhours} className="scrollbar__wrapper" style={{marginRight: '-20px', paddingRight: '20px'}}>
                                                    ...
                                                    <i onClick={(e)=>settimeschoice(e,item.hours+1,index,'hours')} className={`time-spinner__icon ${item.hours==23?'disable':''} down icon`}>
                                                ...
                                                <div className="time-spinner time-picker__minute-spinner">
                                                    <i onClick={(e)=>settimeschoice(e,item.minutes-1,index,'minutes')} className={`time-spinner__icon ${item.minutes==0?'disable':''} up icon`}>
                                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path fillRule="evenodd" d="M7.57253679,6.40074676 L5.1530248,8.66903925 C4.90120463,8.90512066 4.88844585,9.30064304 5.12452726,9.55246321 C5.24268191,9.67849483 5.40773242,9.75 5.58048801,9.75 L10.419512,9.75 C10.76469,9.75 11.044512,9.47017797 11.044512,9.125 C11.044512,8.95224442 10.9730068,8.7871939 10.8469752,8.66903925 L8.42746321,6.40074676 C8.18705183,6.17536109 7.81294817,6.17536109 7.57253679,6.40074676 Z"></path></svg>
                                                    </i>
                                                    <div  className="scrollbar">
                                                        <div onScroll={(e)=>scrooltimes(e,index,'minutes',60)} ref={scrollminutes} className="scrollbar__wrapper" style={{marginRight: '-20px', paddingRight: '20px'}}>
                                                            <div className="scrollbar__content" style={{position:'relative', marginRight: '-20px', paddingRight: '20px'}}>
                                                                <ul className="time-spinner__list">
                                                                    {minutes(60,index,item)}
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <i onClick={(e)=>settimeschoice(e,item.minutes+1,index,'minutes')} className={`time-spinner__icon ${item.minutes==59?'disable':''} down icon`}>
                                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path fillRule="evenodd" d="M7.57253679,9.34925324 L5.1530248,7.08096075 C4.90120463,6.84487934 4.88844585,6.44935696 5.12452726,6.19753679 C5.24268191,6.07150517 5.40773242,6 5.58048801,6 L10.419512,6 C10.76469,6 11.044512,6.27982203 11.044512,6.625 C11.044512,6.79775558 10.9730068,6.9628061 10.8469752,7.08096075 L8.42746321,9.34925324 C8.18705183,9.57463891 7.81294817,9.57463891 7.57253679,9.34925324 Z"></path></svg>
                                                    </i>
                                                </div> 
                                            </div>
                                ...
                                <div className="date-picker-panel__footer">
                                    <button onClick={()=>comfirmdate(index)} className="button-select ">xac nhan</button>
                                </div>
                            </div>
                        </div>
                    </div>
