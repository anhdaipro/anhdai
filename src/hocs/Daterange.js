...
const Daterange=(props)=>{
    const {url,setloading,setdata,showdate,setcount,setDaychoice,daychoice}=props
        ...
        <div ref={parent} data-v-40673d96="" className="date-picker search-time">
            <div className="date-picker__input">
                <div onClick={(e)=>{
                    ...
                    <div className="selector__inner placeholder line-clamp--1">{daychoice?`${timeformat(daychoice.start)} - ${timeformat(daychoice.end)}`:'Bắt Đầu - Kết Thúc'}</div> 
                    <div className="selector__suffix">
                        <i onClick={(e)=>{
                            e.stopPropagation()
                            setStartDate()
                            setEndDate()
                            setDaychoice()}} className="selector__clear-btn icon">
