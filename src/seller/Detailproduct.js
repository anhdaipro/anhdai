...
import {shippingshopURL,detailproductURL,newproductURL} from "../urls"
...
const Detailproduct=()=>{
    ...
    useEffect(() => {
        (async () => {
            if(id){
                ...
                list_media:res.data.media_upload,list_color:res.data.colors,
                list_size:res.data.sizes,max_level:max_level+1})
                setShippingitem([...shipping_item,...shipping_item_remainder])
                setVariation(res.data.variations)
    ...
    useEffect(() => {
        (async()=>{
            if(show){
                if(state.list_category.length==0){
            const res= await axios.get(newproductURL)
                                    ...
                                    <div style={{width:'960px'}} className="modal__content modal__content--normal">
                                        ...
                                        <div className="modal__footer with-assist item-space" style={{justifyContent:'space-between'}}>
                                            ...
                                            <div>
                                                ...
                                                <button onClick={()=>submit()}  className={`btn-m ml-1 ${state.list_choice.some(item=>item.choice)?'':'disable'} btn-orange`} type="button">Next</button>
                                            </div>
