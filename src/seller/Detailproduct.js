...
import {updateimageURL,newproductURL, createvariationURL, detailproductURL,shippingshopURL} from "../urls"
...
const Detailproduct=()=>{
    ...
    const [shipping_item,setShippingitem]=useState([])
    ...
    useEffect(()=>{
        (async()=>{
            ...
            if(!id){
                setShippingitem(datashipping_items)
            }
    ...
    useEffect(() => {
        (async () => {
            const res1= await axios.get(newproductURL)
            const max_level=res1.data.list_category.reduce((arr,obj)=>arr.level>obj.level?arr:obj).level
            ...
            if(id){
                ...
                const list_category=[...res.data.list_category_choice,...list_null]
                const shipping_item=res.data.list_shipping_item.map(item=>{
                    return({...item,enable:true,show:false})
                })
                setCategory(res.data.list_category_choice.find(item=>item.choice))
                setLevel(res.data.list_category_choice.find(item=>item.choice).level)
                ...
                const classify1=res.data.colors.length>0?res.data.colors[0].name:''
                const classify2=res.data.sizes.length>0?res.data.sizes[0].name:''
                setLoading(true)
    ...
    const {list_media,list_choice}=state
    ...
    const setallvariation=()=>{
        const datavariations=variations.map(variation=>{
    ...
    const addcolor=()=>{
        ...
        const datavariations=[...variations,...addvariation]
        setState({...state,list_color:list_color})
        setVariations(datavariations)
    }
    ...
    const setvaluesize=(e,sizechoice)=>{
        ...
        const datavariations=variations.map((variation,index)=>{
                if(variation.size_id==sizechoice.id){
                    return({...variation,size_value:e.target.value})
                }
                return({...variation})
            })
        setState({...state,list_size:list_size})
        setVariations(datavariations)
    }
    ...
    const addsize=()=>{
        ...
        const datavariations=state.list_size.length>0?[...variations,...addvariation]:variations.map(item=>{
    ...
    const setremove=()=>{
        if(state.list_size.length>0){
            const datavariations=variations.reduce((arr,obj)=>{
    ...
    const removesize=(sizechoice)=>{
        ...
        const datavariations=variations.filter(item=>item.size_id!=sizechoice.id)
        setVariations(datavariations)
        setState({...state,list_size:list_size})
    }
            ...
            <Variation>
                <div data-v-a3311712="" className="product-edit-form-item">
                    <div className="product-edit-form-item-content">
                        ...
                        {isNaN(item.price)&& item.price || item.price<1000 && item.price?<div data-v-a3311712="" className="product-edit-form-item-error">{isNaN(item.price)?'Không được để trống ô':'giá lớn hơn 1000'}</div> :''}
                    </div>
                </div>      
                <div data-v-a3311712="" className="product-edit-form-item">
                    <div data-v-a3311712="" className="product-edit-form-item-content">
                        ...
                        {isNaN(item.inventory) && item.inventory?<div data-v-a3311712="" className="product-edit-form-item-error">Không được để trống ô</div> :''}
                    </div>
                </div>
    ...
    const price=useMemo(()=>{
        const price_valid=variations.every(item=>item.price)
        if(variations.length>0 && price_valid){
          return variations.reduce((total,obj)=>{
            return (total+obj.price)/variations.length
          },0)
        }
      },[variations])

    const price_equal=variations.every((item,index,arr)=>item.price && arr.length>0 && item.price==arr[0].price)
    ...
    const submit= async ()=>{
        ...
        }):[{color_id:null,size_id:null,price:formData.price,sku_classify:formData.sku_classify,inventory:formData.inventory}]
    ...
    const setcategorychoice=(i,category)=>{
        ...
        setState(prev=>{return{...prev,list_choice:list_choices}})
    }
    ...
    const setshipshop=(itemchoice,name,value)=>{
        setShipping(current=>current.map(item=>{
            if(item.method==itemchoice.method){
                                        ...
                                        <div className="image_upload item_start">
                                            ...
                                            <div className="edit-main image-manager">
                                                <div className="flex-container">
                                                    ...
                                                    {list_media.filter(media=>media.filetype=='image').length<9?
                                                    ...
                                                    <div data-v-6c83f375="" className="video-container video-player">
                                                        <div data-v-6c83f375="" className="preview">
                                                            <video onEnded={()=>setState({...state,play:false})} ref={videoref} data-v-6c83f375="" controls preload="auto">
                                                                <source data-v-6c83f375="" src={list_media.find(file=>file.file_preview).file} type="video/mp4"/>
                                                            </video> 
                                                            
                                                        </div> 
                                                        <span data-v-6c83f375="" className="process-time">00:00/00:{('0'+list_media.find(file=>file.file_preview).duration).slice(-2)}</span> 
                                        ...
                                        <div className="item_start mb-2">
                                            ...
                                            <div className="edit-input edit-main">
                                                <div className="input__inner input__inner--large"> 
                                                    <input type="text" className="form-select name" style={{width:'100%'}} name="name" minLength="10" maxLength="120" onChange={(e)=>setformData({...formData,name:e.target.value})} value={formData.name} id="nameproduct" placeholder="Name Product"  required/>
                                            ...
                                            <div onClick={()=>setShow(true)} className="edit-input edit-main">
                                                <div className="input__inner input__inner--large" style={{display:'flex'}}>
                                                    <div className="item-center">
                                                        {list_choice.filter(item=>item.id).map((item,i)=> 
                                                            <span data-v-188571b4="" data-v-5bb79eae="" className="cat-selected-wrapper">
                                                                ...
                                                                <span data-v-188571b4="" style={{display:`${i==list_choice.filter(item=>item.id).length-1?'none':''}`}} data-v-5bb79eae="" className="spacer-point">&gt;</span>
                                                            </span>
                                                        )}
                                                </div>
                                                </div> 
                                                                    ...
                                                                    <ContentRight>
                                                                        ...
                                                                        <Bodytable>
                                                                            ...
                                                                            <Flexcol key={color.id}>
                                                                                {variations.filter(variation=>variation.color_id==color.id).map(variation=>
                                            ...
                                            <div className="edit-input edit-text-mini">
                                                <div className="d-flex">
                                                    <div className='product-dimension-edit-item product-edit-form-item'>
                                                        <div className="product-edit-form-item-content">
                                                            <div className="input__inner input__inner--large">
                                                                    <input onChange={(e)=>setdelivery('width',e)} type="text" className="form-select" value={formData.width?formData.width:''} name=" width" placeholder="R" required/>
                                                                    <div className="input__suffix">
                                                                        <span className="input__suffix-split"></span>
                                                                        cm
                                                                    </div>
                                                    ...
                                                    <div className='product-dimension-edit-item product-edit-form-item'>
                                                        <div className="product-edit-form-item-content">
                                                            <div className="input__inner input__inner--large">
                                                                <input onChange={(e)=>setdelivery('length',e)} type="text" className="form-select " value={formData.length?formData.length:''}  name="length" placeholder="D"  required/>
                                                                <div className="input__suffix">
                                                                    <span className="input__suffix-split"></span>
                                                                    cm
                                                                </div>
                                                    ...
                                                    <div className='product-dimension-edit-item product-edit-form-item'>
                                                        <div className="product-edit-form-item-content">
                                                            <div className="input__inner input__inner--large">
                                                                <input onChange={(e)=>setdelivery('height',e)} type="text" className="form-select" value={formData.height?formData.height:''}  name="height" placeholder="C" required/>
                                                                <div className="input__suffix">
                                                                    <span className="input__suffix-split"></span>
                                                                    cm
                                                                </div>
                                        ...
                                        <div className="edit-row d-flex">
                                            ...
                                            <div className="edit-input edit-logistic-container"> 
                                                ...
                                                <div className="logistics-section logistics-section--enabled" >
                                                    {shipping.map((ship,i)=>
                                                    <div key={i} className="logistics-item logistics-item-ui-t3">{/*add logistics-item  transform: svg rotate(90deg)*/}
                                                        <div className="optional-item d-flex">
                                                            <div onClick={(e)=>setshipshop(ship,'show',!ship.show)} className={`expend-button ${ship.show?'is-expend':''}`}>
                                                                <i className="icon">
                                                            ...
                                                            <div className="item-center col--grow">
                                                                <div className="col logistics-item-name text-overflow">{ship.method}</div>
                                                            ...
                                                            <div data-v-5e61e860="" className="inline-list-item">
                                                                ...
                                                                <>
                                                                <div data-v-4f5fa871="" data-v-5e61e860="" className="price">₫{formatter.format(price_ship[ship.method].find(item=>item.weight_from<=formData.weight && item.weight_to>=formData.weight).price)}</div> 
                                                                ...
                                                                <div className={`switch_box ${!formData.weight?'disable':''}`}>
                                                                    <input onChange={()=>setshipping(ship.method)} type="checkbox" disabled={!formData.weight?true:false} name="check"  checked={shipping_item.some(item=>item.enable && item.method==ship.method)?true:false} className="switch_1"/>
                                                                </div>
                                                            </div>
                                                        </div> 
                                                        <div data-v-5e61e860="" style={{display:`${!ship.show?'none':''}`}}>
                                                            {ship.unit.map((item,j)=>
                                                            <div data-v-5e61e860="" key={j} className="actual-channel">
                                                                <span data-v-5e61e860="">{item.shipping_unit} 
                                                ...
                                                </div>:<Link to="/vendor/shipping/list" className="btn-link mb-1"><span>cài đặt đơn vị vận chuyển</span></Link>}
                                                ...
                                                <div className="category-selector">
                                                    <div style={{display:'block'}} className="selector">
                                                        ...
                                                        <div className="category-wrap">
                                                            <div onClick={()=>setLeft(left+168.5)} className="btn-wrap-left" style={{display: `${level<3 || (level>=3 && left==0)?'none':'block'}`}}>
                                                            ...
                                                            <div onClick={()=>setLeft(left-168.5)} className="btn-wrap-right" style={{right: '0px', display: `${level<3 || (level>=3 && left<0)?'none':'block'}`}}>
                                                                <button type="button" className="cat-btn">
                                                            ...
                                                            <div className="category-list" style={{left: `${left}px`,width: '1200px'}}>
